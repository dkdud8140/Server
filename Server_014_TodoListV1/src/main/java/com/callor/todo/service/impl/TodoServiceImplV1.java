...
public class TodoServiceImplV1 implements TodoService {
	...
	/*
	 * 객체(Entity, Relation) DataBase를 만들고 Table을 생성한 후 Insert 수행하면 실제 상황의 데이터가
	 * Table에 저장된다 Insert 수행한 후 Select를 수행하여 조회를 할 수 있는 데이터들
	 *
	 * 메타데이터(Meta Data) 생성한 Database, table, sequence, view 등등이 어떤 구조로 만들어져 있는가 등의
	 * 정보를 메타데이터라고 한다. ex) myDB라는 database에 몇 개의 table이 있는가 등의 정보를 알고 싶으면 myDB
	 * 메타데이터를 참조하면된다.
	 * 
	 */
	protected List<Map<String, Object>> select(ResultSet rSet) throws SQLException {

		// ResultSet으로 메타데이터를 추출하여 조회를 수행한 결과로 전달받은 Table에
		// 몇 개의 칼럼이 있는지 확인
		...
		while (rSet.next()) {
			...
			for (int i = 0; i < columns; i++) {
				System.out.println(rSet.getObject(i + 1));
