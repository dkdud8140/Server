 ...
 * DB와 연동한 Service
 * 
 * DB연결을 하고 (중요!!) // 한 번 연결이 되면 나머지 테이블에서 계속 사용이 가능하도록
 * 
 * SQL을 작성하고
 * SQL Packing하고
...
public class BookServiceImplV1 implements BookService {
	...
	public void insert(BookVO bookVO) {
		...
		//sql +=  "VALUES (" + bookVO.getBk_isbn() + "," + bookVO.getBk_title() + "," + bookVO.getBk_ccode() + "," + ....
		...
		try {
			...
			pStr.executeUpdate(); // DB에 insert 된다.
	...
	public BookDTO findById(String bk_isbn) {
		...
		try {
			...
			if(result.next()) {
				...
				bookDTO.setBk_autor(result.getString("출판사대표"));
