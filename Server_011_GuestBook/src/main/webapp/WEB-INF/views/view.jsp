...
<link href="${rootPath}/static/css/home.css?ver2021-05-18-00" rel="stylesheet"/>
	<style>
		div.view_btn {
			width : 80 %;
			margin: 10px auto ;
			...
			document.querySelector("div.view_btn").addEventListener("click",function(ev){
				...
				if(className == "btn_home") {
					...
					if(confirm("방명록을 삭제합니다")) {
						...
						href는 새로운 페이지를 열고 URL에서 res한 정보를 보여준다
						새로운 페이지에서 뒤로가기를 하면 현재 페이지로 되돌아 올 수 있다,
						
						replace()는 현재 페이지에서 URL에서 res한 
						*/
						
						document.replace("${rootPath}/guest/delete?gb_seq="+${GB.gb_seq});
					}
				}
			})
		})
